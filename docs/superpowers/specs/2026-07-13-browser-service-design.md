...
├── pacing.py         # 실사이트 조작 간격과 재시도 대기 강제
...
- `pacing.py`: 네이버 실사이트 조작 직렬화와 최소 대기 시간 보장
...
- `BrowserAccessBlockedError`: 403, 429, CAPTCHA, 비정상 접근 제한 화면 감지
...
재시도 전에는 최소 5초 기다린다. `BrowserAccessBlockedError`는 추가 요청을 만들지
않도록 상태 복원과 자동 재시도를 생략하고 즉시 상위 호출자에게 반환한다.
...
네이버 실사이트에 대한 자동화 조작은 `InteractionPacer`를 반드시 거친다.

- 검색 제출, 역 선택, 줌 변경, 후보 선택, 탭 선택, 사진 분류 선택, 리뷰 더보기,
  패널 닫기 등 상태를 바꾸는 UI 조작 사이에 최소 3초 대기
- 단조 증가 시계와 비동기 lock을 사용해 여러 세션의 실사이트 조작도 직렬화
- 재시도는 일반 조작 간격과 별도로 최소 5초 대기
- 실사이트 스모크 시나리오는 한 번에 하나만 실행
- 동일 스모크 시나리오 연속 실행 사이에 최소 30초 대기
- 실사이트 스모크는 명시적 실행만 허용하고 기본 테스트 탐색과 CI에서는 제외
- 403, 429, CAPTCHA, 비정상 접근 제한 문구 감지 시 즉시 실행 중단

`InteractionPacer`는 기본값을 낮추거나 비활성화할 수 없게 한다. 단위 테스트와
로컬 HTML 통합 테스트는 가짜 clock과 sleeper를 주입해 실제 대기 없이 간격 호출
자체를 검증한다. 네이버 호스트를 사용하는 실사이트 테스트만 실제 시간을 기다린다.
...
- 예외 타입과 오류 컨텍스트 확인
- 첫 실패 후 정확히 1회만 재시도
- 실사이트 UI 조작 사이에 최소 3초 간격 요청
- 재시도 전 최소 5초 대기
- 여러 세션의 실사이트 조작 직렬화
- 접근 제한 감지 시 대기 중 조작 취소와 무재시도
...
- 상세 종료 후 검색 목록 복원
- 중간 실패 시 재시도와 자원 정리
- 가짜 clock과 sleeper를 사용한 pacing 호출 검증
...
- 모든 상태 변경 조작 사이에 실제 3초 이상 경과함
- 재실행 시 이전 실행 종료 후 실제 30초 이상 경과함
- 접근 제한 신호가 발생하면 추가 조작 없이 종료됨

실사이트 테스트는 네이버 UI와 네트워크에 의존하므로 기본 단위 테스트와 분리해
명시적으로 한 번씩 실행한다. 테스트 실패를 확인하기 위한 반복 실행도 30초 간격을
그대로 적용한다.
...
- 실사이트 조작 3초, 재시도 5초, 시나리오 재실행 30초 간격이 강제됨
- 접근 제한 감지 시 자동 재시도 없이 즉시 중단됨
